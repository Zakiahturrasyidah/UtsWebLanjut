...
// ===============================
// AMBIL SEMUA KURSI (NATURAL ORDER)
// ===============================
...
// ===============================
// AMBIL KURSI YANG SUDAH BOOKED (confirmed)
// ===============================
$booked = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['book'])) {
    ...
    if (empty($selected_arr)) {
        ...
        try {
            // Ambil semua booked lagi di dalam transaksi (double-check)
            ...
            if (!empty($conflict)) {
                ...
                // juga cek kursi non-active (isActive = 0) => tidak boleh dipesan
                // ambil status kursi yang dipilih
                $placeholders = implode(',', array_fill(0, count($selected_arr), '?'));
                // prepare dynamic query: SELECT seatCode, isActive FROM seats WHERE studioId=? AND seatCode IN (...)
                ...
                // bind params dynamic: first studio_id (i) then seat codes (s...)
                $bindParams = [];
                $bindTypes = 'i' . $types;
                ...
                // we need correct binding — use reflection helper
                $refs = [];
                $refs[] = $bindTypes;
                $refs[] = &$studio_id;
                foreach ($selected_arr as $k => $code) {
...
seats.forEach(s => {
    if (s.dataset.disabled === '1') {
        // visually non-clickable already; skip adding listener
        return;
    }
...
// pre-fill if user wants to keep selection between errors (optional)
syncInput();
</script>
